...
# On écrit l'en-tête du CSV
echo "SeqID,Length" > outputs/read_lengths.csv

# On lance awk
awk 'NR % 4 == 1 { id = substr($1, 2) } 
     NR % 4 == 2 { print id "," length($0) } 
    ' $FASTQ_FILE >> outputs/read_lengths.csv
